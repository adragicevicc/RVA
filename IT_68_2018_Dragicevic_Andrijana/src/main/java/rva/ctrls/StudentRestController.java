...
public class StudentRestController {
	...
	@GetMapping("studentiUGrupi/{id}")
	@ApiOperation(value = "Vraca kolekciju svih studenata iz baze podataka kod kojih grupa ima vrednost id koji je prosledjen kao path varijabla")
	public Collection<Student> studentPoGrupiId(@PathVariable("id") Integer id) {
		Grupa g = grupaRepository.getOne(id);
		return studentRepository.findByGrupa(g);
	}
	
	@GetMapping("studentBrojIndeksa/{broj_indeksa}")
	@ApiOperation(value = "Vraca kolekciju svih studenata iz baze podataka ciji broj indeksa sadrzi string koji je prosledjen kao path varijabla")
	public Collection<Student> getStudentByBrojIndeksa(@PathVariable("broj_indeksa") String broj_indeksa) {
	...
	public ResponseEntity<Student> deleteStudent(@PathVariable("id") Integer id) {
		...
		if(id == -100) {
			jdbcTemplate.execute(
					...
					+ "VALUES(-100, 'ImeTest', 'PrezimeTest', 'BrIndeksaTest', 1, 1)"
					);
		}
		return new ResponseEntity<Student>(HttpStatus.OK);
	}
